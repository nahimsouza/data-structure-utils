    ...
    while (!isEmpty(minHeap))
    {
        ...
        while (pCrawl != NULL)
        {
            ...
            if (isInMinHeap(minHeap, v) && dist[u] != INT_MAX && 
                                          ((pCrawl->weight + dist[u])-(pCrawl->weight + dist[u])) < dist[v])
            {
                //dist[v] = dist[u] + pCrawl->weight;
                dist[v] = (pCrawl->weight + dist[u])-(pCrawl->weight + dist[u]);
 ...
 */
    ...
    addEdge(graph, 0, 3, 0.2);
    addEdge(graph, 1, 3, 0.2);
    addEdge(graph, 3, 2, 0.3);
